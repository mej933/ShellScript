...
#Writer: MaxdSre
#Date: Oct 30, 2017 19:32 Sat +0800 - add all distros release info print function
...
#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'LifeCycleTemp_XXXXXX'}
...
while getopts "ac:dmp:h" option "$@"; do
    case "$option" in
        a ) all_list=1 ;;
...
funcLinuxDistributionMenuList(){
    ...
    echo "${c_blueb}Available GNU/Linux Distribution List:${c_normal}"
    ...
    awk -F\| '{if(a[$1]==""){a[$1]=$1}}END{PROCINFO["sorted_in"]="@ind_num_desc";for (i in a) print i}' "${rhel_eus_date}" "${rhel_release_date}" | while read -r version_specific; do
        ...
        if [[ -n "${specific_version_info}" ]]; then
            echo "${specific_version_info}" | while IFS="|" read -r version release errate kernel;do
                if [[ "${all_list}" -ne 1 ]]; then
                    ...
                    if [[ -n "${specific_eus_date}" ]]; then
                        is_eol=1
                        [[ $(date --date="${specific_eus_date}" +'%s') -gt ${current_timestamp} ]] && is_eol=0
                    fi

                    echo "rhel|${version_specific}||${release}|${specific_eus_date}|${is_eol}"
                fi
            ...
            if [[ "${all_list}" -ne 1 ]]; then
                ...
                echo "rhel|${version_specific}|||${specific_eus_date}|1"
            fi
...
funcLifeCycleCentOS(){
    ...
    ${download_tool} "${announce_archive_site}" | sed -r -n '/Downloadable/,/\/table/{/(Thread|Subject|Author|Gzip|table|<tr>)/d;s@<\/?td>@@g;s@^[[:space:]]*@@g;/^$/d;p}' | sed -r ':a;N;$!ba;s@\n@ @g;s@<\/tr>@\n@g;' | sed -r -n '/href/{s@^[[:space:]]*@@g;s@([^:]*):.*\"(.*)date.html\".*@'"${announce_archive_site}"'\2@g;p}' | parallel -k -j 0 funcCentOSSpecificReleaseInfo 2> /dev/null >> "${centos_announce_archive}"
    ...
    awk -F\| '{!a[$1]++;arr[$1]=$0}END{PROCINFO["sorted_in"]="@ind_str_desc";for (i in arr) print arr[i]}' "${centos_release_date}" | while IFS="|" read -r release_no release_date release_archive_page; do
        ...
        if [[ "${show_details}" -eq 1 ]]; then
            ...
            if [[ "${all_list}" -ne 1 ]]; then
                ...
                if [[ -n "${eus_date}" ]]; then
                    is_eol=1
                    [[ $(date --date="${eus_date}" +'%s') -gt ${current_timestamp} ]] && is_eol=0
                else
                    is_eol=1
                    [[ "${release_no%%.*}" -eq 6 ]] && is_eol=0
                fi

                echo "centos|${release_no}||${release_date%% *}|${eus_date}|${is_eol}"
            fi
...
funcLifeCycleDebian(){
    ...
    local debian_release_page='https://www.debian.org/releases/'
    ...
    ${download_tool} "${debian_release_page}" | sed -r -n '/Index of releases/,/<\/ul>/{p}' | awk '$0~/stable release/{print gensub(/.*href="([^"]+)">[^[:digit:]]+([[:digit:].]+) .*$/,"'"${debian_release_page}"'\\1 \\2","g",a)};{a=$0}' | awk '$2>6' | while IFS=" " read -r s_link s_version; do
        # https://www.debian.org/releases/stretch/ 9
        local extract_release_info=''
        extract_release_info=$(${download_tool} "${s_link}" | sed -r -n '/Release Information<\/h1>/,/included many major/{/h1/d;/include/d;s@<[^>]+>@@g;s@(initially|on) @@g;p}' | sed -r -n ':a;N;$!ba;s@\n@ @g;s@[[:space:]]*was released[[:space:]]*@|@g;s@\. Debian[[:space:]]*@|@g;s@Debian[[:space:]]*@@g;s@([[:digit:]])[[:alpha:]]+,@\1,@g;s@\.$@@g;p')
        ...
        if [[ -z "${extract_release_info}" ]]; then
            ...
            local new_release_version=$(echo "${extract_release_info}" | awk -F\| '{print $1}')
            local new_release_date=$(echo "${extract_release_info}" | awk -F\| '{print $2}')
            local major_release_version=$(echo "${extract_release_info}" | awk -F\| '{print $3}')
        ...
        if [[ "${show_details}" -eq 1 ]]; then
            ...
            if [[ "${all_list}" -ne 1 ]]; then
                ...
                if [[ "${l_eol_date}" =~ ^approx ]]; then
                    ...
                    [[ $(date --date="01 ${l_eol_date} next month last day" +'%s') -gt ${current_timestamp} ]] && is_eol=0
                else
                    [[ $(date --date="${l_eol_date}" +'%s') -gt ${current_timestamp} ]] && is_eol=0
                fi

                echo "debian|${release_version}|${codename,,}|${releases_date}|${l_eol_date}|${is_eol}"
            fi
                ...
                if [[ "${release_version%%.*}" -lt 13 ]]; then
                    ...
                    if [[ -n "${eol_date}" ]]; then
                        ...
                        if [[ "${eol_date}" =~ ^[a-zA-Z] ]]; then
                            [[ $(date --date="01 ${eol_date} next month last day" +'%s') -gt ${current_timestamp} ]] && is_eol=0
                        else
                            [[ $(date --date="${eol_date}" +'%s') -gt ${current_timestamp} ]] && is_eol=0
                        fi
                    fi
                fi

                echo "ubuntu|${release_version%% *}|${codename,,}|${release_date}|${eol_date}|${is_eol}"
            fi
                ...
                if [[ "${release_version%%.*}" -lt 13 ]]; then
                    ...
                    if [[ -n "${l_eol_left_date}" ]]; then
                        ...
                        if [[ "${l_eol_left_date}" =~ ^[a-zA-Z] ]]; then
                            [[ $(date --date="01 ${l_eol_left_date} next month last day" +'%s') -gt ${current_timestamp} ]] && is_eol=0
                        else
                            [[ $(date --date="${l_eol_left_date}" +'%s') -gt ${current_timestamp} ]] && is_eol=0
                        fi
                    fi
                fi

                echo "ubuntu|${release_version%% *}|${codename,,}|${release_date}|${eol_left_date}|${is_eol}"
