#!/usr/bin/env bash
# shellcheck disable=SC2034,SC2031,SC2030
...
#Writer: MaxdSre
#Date:
#Update Time: Dec 29, 2017 12:49 Fri +0800 - Change output style
...
#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'ODDTemp_XXXXXX'}
...
funcFileDownloadCoreOperation(){
    ...
    if [[ -s "${l_doc_save_name}" ]]; then
        ...
        echo -e "... (${c_yellow}$(ls -hs "${l_doc_save_name}" | awk '{print $1}')${c_normal}) [${c_green}ok${c_normal}]"
        ...
        if [[ "${l_retry_count}" -le "${max_retry_count}" ]]; then
            ...
            let l_retry_count+=1
            # iteration
            funcFileDownloadCoreOperation "${l_category_name}" "${l_doc_name}" "${l_doc_url}" "${l_doc_save_name}" "${l_retry_count}"
        else
...
funcFileDownloadProcedure(){
    ...
    if [[ -n "${l_category_name}" && -n "${l_doc_name}" && -n "${l_doc_url}" && -n "${l_doc_save_name}" ]]; then
        ...
        funcFileDownloadCoreOperation "${l_category_name}" "${l_doc_name}" "${l_doc_url}" "${l_doc_save_name}"
    fi
}
...
funcDocSaveDirectoryStatement(){
    ...
    funcCommandExistCheck 'nautilus' && nautilus "${l_target_dir}" &
}
...
funcRedHatProductPageLinkExtraction(){
    ...
    echo "${c_blueb}Available Product Topic List:${c_normal}"
    ...
    echo "${c_blueb}Available Product List Under ${choose_product_topic}:${c_normal}"
    ...
    if [[ -n "${version_list}" ]]; then
        ...
        echo "${c_blueb}Available Version List Of ${choose_product}:${c_normal}"
    ...
    if [[ "${category_manual_choose}" -eq 1 ]]; then
        ...
        if [[ -n "${category_list}" ]]; then
            ...
            echo "${c_blueb}Available Category List Of ${choose_product} ${choose_version}:${c_normal}"
...
funcSUSEProductPageLinkExtraction(){
    ...
    echo "${c_blueb}Available Product List:${c_normal}"
    PS3="${c_yellow}Choose product number(e.g. 1, 2,...): ${c_normal}"
    ...
    echo "${c_blueb}Available Version List:${c_normal}"
    ...
    if [[ "${category_manual_choose}" -eq 1 ]]; then
        ...
        echo "${c_blueb}Available category List:${c_normal}"
...
funcOpenSUSEProductPageLinkExtraction(){
    ...
    echo "${c_blueb}Available Release Version List:${c_normal}"
    PS3="${c_yellow}Choose version number(e.g. 1, 2,...): ${c_normal}"
...
funcAWSOperation(){
    funcAWSProductPageLinkExtraction
    # - Download Procedure
    local target_save_dir="${doc_save_dir}/Amazon Web Services Documentation"
    ...
    $download_tool "${product_page_url}" | sed -r -n '/<tbody>/,/<\/tbody>/{s@<br \/>@@g;s@<\/a>@&\n@g;/href=/!d;p}' | sed -r -n 's@.*href="([^"]+)"[^>]*>([^<]+)<.*$@\2|\1@g;s@^[[:blank:]]*@@g;/\|/!d;/^\|/d;/Kindle/d;/^(PDF|HTML)/!{s@.*@---\n&@g};p' | sed -r -n ':a;N;$!ba;s@\n@|@g;s@\|?---\|?@\n@g;p' | sed -r '/HTML/!d' | while IFS="|" read -r title url html html_url pdf pdf_url; do
        ...
        if [[ -n "${file_url}" && "${file_url}" =~ ${file_type,,}$ ]]; then
            doc_save_name="${category_dir}/${counter_flag} - ${title//\//&}.${file_url##*.}"
            [[ -f "${doc_save_name}" ]] && rm -f "${doc_save_name}"
            funcFileDownloadProcedure "${category_dir##*Amazon Web Services Documentation/}" "${title}" "${file_url}" "${doc_save_name}"
            let counter_flag+=1
        fi    # end if
