...
#Writer: MaxdSre
#Date: Jan 25, 2018 12:10 Thu +0800 - ssh connection info detection optimization
#Update Time:
...
# - Sep 21, 2017 10:40 Thu +0800


#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'PUIDTemp_XXXXXX'}
...
funcSSHSessionInfo(){
    ...
    # http://www.putorius.net/2014/08/explaination-of-sshnotty-rootnotty-or.html
    ...
    case "${l_service_info_origin}" in
        ...
        ssh )
            ...
            l2r_user_info=$(echo "${l2r_user_info}" | sed -r -n 's@[[:space:]]*\[[^]]+\][[:space:]]*@@g;p')
            l2r_host_info="${l2r_user_info##* }"

            if [[ "${l2r_host_info}" =~ @ ]]; then
                l2r_host_info="${l2r_host_info##*@}"
                service_info="${l_service_info_origin} ${l2r_host_info}"
            elif [[ "${l2r_host_info}" =~ ^([0-9]{1,3}.){3}[0-9]{1,3}$ ]]; then
                service_info="${l_service_info_origin} ${l2r_host_info}"
...
funcCoreProcessingProcedure(){
    ...
    while IFS=" " read -r protocol port process_state service_name process_id; do
        ...
        service_info_origin=$(awk 'match($1,/^'"${process_id}"'$/){cmd_name=$3;cmd_path=gensub(/.* [[:digit:]].[[:digit:]] (.*)$/,"\\1","g",$0);print cmd_path}' "${process_list}" | sed -r 's@.*-Dcatalina.home=([^[:space:]]+).*@\1@g;s@.*-Djetty.home=(.*) -Djetty.base.*@\1@;s@.*QuorumPeerMain (.*)/bin.*@\1@g;s@.*-Dproject.dir=(.*)( .*)?@\1@g;s@.*-Des.path.home=([^[:space:]]+).*@\1@g;;s@^([^[:space:]]+).*@\1@g;s@:$@@g;')
