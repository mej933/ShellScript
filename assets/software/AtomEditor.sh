...
#Writer: MaxdSre
#Date: Nov 28, 2017 17:26 Tue +0800
#Update Time:
# - Sep 21, 2017 17:45 Thu +0800
# - Nov 20, 2017 09:08 Mon +0800

#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'ATETemp_XXXXXX'}
...
funcInitializationCheck(){
    ...
    atom_config_path=${atom_config_path:-"${user_home}/.atom"}
...
funcVersionLocalCheck(){
    if [[ -s "${installation_dir}/atom" ]]; then
        current_version_local=$("${installation_dir}/atom" --version 2>/dev/null | sed -r -n '/^Atom/{s@[^[:digit:]]*(.*)$@\1@g;p}')
...
funcVersionOnlineCheck(){
    ...
    if [[ "${is_existed}" -eq 1 ]]; then
        ...
        if [[ -s /etc/debian_version ]]; then
            if funcCommandExistCheck 'apt'; then
                # https://github.com/atom/atom#archive-extraction
                local pack_list
                pack_list=$($download_tool https://github.com/atom/atom | sed -r -n '/Install dependencies/{s@.*install[[:space:]]*([^<]*)<.*@\1@g;p}')
                [[ -z "${pack_list}" ]] && pack_list='git gconf2 gconf-service libgtk2.0-0 libudev1 libgcrypt20 libnotify4 libxtst6 libnss3 python gvfs-bin xdg-utils libcap2'
                # shellcheck disable=SC2086
                apt-get install ${pack_list} -yq 1> /dev/null
            fi
        fi    # end if debian_version
...
funcDownloadAndDecompressOperation(){
    ...
    tar xf "${l_package_path}" -C "${installation_dir}" --strip-components=1    # Decompress To Target Directory

    # Installed Version In System
    [[ -f "${pack_save_path}" ]] && rm -f "${pack_save_path}"
...
funcExecuationPathConfiguration(){
    ...
    echo "export PATH=${installation_dir}/atom:\$PATH" > "${execute_path}"

    apm_path=${apm_path:-"${installation_dir}/resources/app/apm/bin/apm"}
...
funcPluginsInstallation(){
	...
	chown -R "${now_user}" "${atom_config_path}"

tee "${atom_config_path}/package.txt" &> /dev/null <<-'EOF'
atom-runner
autocomplete-python
emmet
file-icons
...
# apm install --packages-file ~/.atom/package.txt
    ...
    if [[ -s "${apm_path}" ]]; then
        printf "${c_red}Attention${c_normal}: if you wanna install plug-in, run the follow command in terminal:\n${c_blue}%s${c_normal}\n\n" "${apm_path} install --packages-file ${atom_config_path}/package.txt"
        chown -R "${now_user}" "${atom_config_path}/package.txt"
    fi
}
...
tee "${atom_config_path}/config.cson" &> /dev/null <<-'EOF'
"*":
  "autocomplete-python":
    enableTouchBar: true
    showTooltips: true
