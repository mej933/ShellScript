...
#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'FZTemp_XXXXXX'}
...
funcDownloadToolCheck(){
    ...
    if funcCommandExistCheck 'curl'; then
        download_tool="curl -fsL --retry ${retry_times} --retry-delay ${retry_delay_time} --connect-timeout ${connect_timeout_time} --no-keepalive"   # curl -s URL -o /PATH/FILE； -fsSL
        ...
        download_tool="wget -qO- --tries=${retry_times} --waitretry=${retry_delay_time} --connect-timeout ${connect_timeout_time} --no-http-keep-alive" # wget -q URL -O /PATH/FILE
...
funcVersionLocalCheck(){
    if [[ -f "${installation_dir}/bin/filezilla" ]]; then
        is_existed=1
        current_version_local=$("${installation_dir}/bin/filezilla" --version 2> /dev/null | sed -r -n 's@.* ([[:digit:].]*) .*@\1@p')
    fi
}
...
funcDownloadAndDecompressOperation(){
    ...
    new_installed_version=$("${installation_dir}/bin/filezilla" --version 2> /dev/null | sed -r -n 's@.* ([[:digit:].]*) .*@\1@p')   # Just Installed Version In System
...
Exec=installation_dir/bin/filezilla
Terminal=false
...
funcDesktopConfiguration(){
    if [[ -d '/usr/share/applications' ]]; then
        [[ -f "${installation_dir}/share/icons/hicolor/48x48/apps/filezilla.png" ]] && ln -sf "${installation_dir}/share/icons/hicolor/48x48/apps/filezilla.png" "${pixmaps_png_path}"
        funcDesktopFileConfiguration
    fi
