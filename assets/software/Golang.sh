...
#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'GoTemp_XXXXXX'}
...
funcVersionLocalCheck(){
    if [[ -f "${installation_dir}/VERSION" ]]; then
        is_existed=1
        # current_version_local=$(awk '{print gensub(/^go(.*)/,"\\1","g",$0)}' $installation_dir/VERSION)
        current_version_local=$(sed -r -n 's@^go(.*)$@\1@p' "${installation_dir}/VERSION")
    fi
}
