...
#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'LOTemp_XXXXXX'}
...
funcInstallationOperation(){
    ...
    funcPackageDownload "${main_pack_link}" "${main_pack_save_name}"
    funcPackageDownload "${help_pack_link}" "${help_pack_save_name}"
    [[ "${lang_pack_existed}" -eq 1 ]] &&  funcPackageDownload "${lang_pack_link}" "${lang_pack_save_name}"
    ...
    funcDecompressTargetFile "${main_pack_save_name}" "${main_pack_extract_path}" "${package_type}"
    funcDecompressTargetFile "${help_pack_save_name}" "${help_pack_extract_path}" "${package_type}"
    [[ "${lang_pack_existed}" -eq 1 ]] && funcDecompressTargetFile "${lang_pack_save_name}" "${lang_pack_extract_path}" "${package_type}"
