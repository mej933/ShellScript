...
#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'MFTemp_XXXXXX'}
...
funcVersionLocalCheck(){
    if [[ -f "${installation_dir}/firefox" ]]; then
        is_existed=1
        current_version_local=$("${installation_dir}/firefox" --version | awk '{print $NF}')
    fi
}
...
funcDownloadAndDecompressOperation(){
    ...
    new_installed_version=$("${installation_dir}/firefox" --version | awk '{print $NF}')    # Just Installed Version In System
...
Exec=installation_dir/firefox %u
...
funcDesktopConfiguration(){
    if [[ -d '/usr/share/applications' ]]; then
        [[ -f "${installation_dir}/browser/icons/mozicon128.png" ]] && ln -sf "${installation_dir}/browser/icons/mozicon128.png" "${pixmaps_png_path}"
        funcDesktopFileConfiguration
    fi
