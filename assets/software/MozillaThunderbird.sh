...
#Official Site: https://www.mozilla.org/en-US/thunderbird/
#Target: Automatically Install & Update Mozilla Thunderbird On GNU/Linux
#Writer: MaxdSre
#Date: Jun 2, 2017 08:43 Fri +0800
#Update Time:
# - Feb 15, 2017 09:42 Wed +0800
# - May 15, 2017 11:03 Mon -0400


#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'MTemp_XXXXXX'}
...
readonly official_site='https://www.mozilla.org/en-US/thunderbird/'   #Mozilla Thunderbird Official Site
readonly download_page="${official_site}all/"        # Download Page
readonly release_note_page="${official_site}notes/"
software_fullname=${software_fullname:-'Mozilla Thunderbird'}
...
application_name=${application_name:-'mozillaThunderbird'}
...
funcVersionLocalCheck(){
    if [[ -f "${installation_dir}/thunderbird" ]]; then
        is_existed=1
        current_version_local=$("${installation_dir}/thunderbird" --version | awk '{print $NF}')
    fi
}

funcVersionOnlineCheck(){
    latest_version_online=$($download_tool "${release_note_page}" | sed -r -n '/<h1>Thunderbird/{n;s@.*Version ([^,]*),.*@\1@p}')
    release_date=$($download_tool "${release_note_page}" | sed -r -n '/<h1>Thunderbird/{n;s@.*on ([^<]*).*@\1@p}')
...
funcUninstallOperation(){
    ...
    local config_path="${user_home}/.thunderbird"
    [[ -d "${config_path}" ]] && rm -rf "${config_path}"    # ~/.thunderbird
    local cache_path="${user_home}/.cache/thunderbird"
...
funcDownloadAndDecompressOperation(){
    ...
    local pack_save_path="${temp_save_path}/thunderbird-${latest_version_online}.tar.bz2"
    [[ -f "${pack_save_path}" ]] && rm -f "${pack_save_path}"

    [[ "${change_language}" -eq 1 ]] && lang='zh-TW'
    $download_tool "https://download-installer.cdn.mozilla.net/pub/thunderbird/releases/${latest_version_online}/${os_type}/${lang}/thunderbird-${latest_version_online}.tar.bz2" > "${pack_save_path}"     # Download .tar.bz2 Installation Package
    ...
    new_installed_version=$("${installation_dir}/thunderbird" --version | awk '{print $NF}')    # Just Installed Version In System
...
funcDesktopConfiguration(){
    if [[ -d '/usr/share/applications' ]]; then
        [[ -f "${installation_dir}/chrome/icons/default/default256.png" ]] && ln -sf "${installation_dir}/chrome/icons/default/default256.png" "${pixmaps_png_path}"
        funcDesktopFileConfiguration
    fi
