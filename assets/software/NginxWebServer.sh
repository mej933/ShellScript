...
#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'NWSTemp_XXXXXX'}
...
readonly conf_nginx_conf="${custom_shellscript_url}/master/configs/nginx/nginx.conf"
readonly conf_default_conf="${custom_shellscript_url}/master/configs/nginx/default.conf"
    ...
    case "${distro_name}" in
        rhel|centos )
            if [[ ${version_id%%.*} -ge 7 ]]; then
                ...
                funcSystemServiceManager 'firewalld' 'enable'
        ...
        sles|opensuse )
            ...
            funcSystemServiceManager 'SuSEfirewall2' 'enable'
...
funcOperationProcedure(){
    ...
    case "${distro_name}" in
        rhel|centos )
            ...
            case "${action}" in
                0 )
                    [[ -f "${repo_file_path}" ]] && cp -fp "${repo_file_path}" "${repo_file_path}${bak_suffix}"
                ...
                3 )
                    ...
                    if [[ -f "${repo_file_path}${bak_suffix}" ]]; then
                        mv "${repo_file_path}${bak_suffix}" "$repo_file_path"
                    else
        ...
        debian|ubuntu )
            ...
            case "${action}" in
                0 )
                    ...
                    [[ "${distro_name}" == 'ubuntu' && "${version_id}" == '17.10' ]] && codename='zesty'    # currently nginx not provide artful(17.10) repo
...
funcVersionOnlineCheck(){
    ...
    if [[ "${update_upgrade_enable:-0}" -eq 1 ]]; then
        ...
        if [[ "${release_type_local}" == 'stable' && "${release_type}" == 'mainline' ]]; then
            funcOperationProcedure 'upgrade'
        else
...
funcPostInstallationConfiguration(){
    # "${BASH_VERSINFO[0]}" -ge 4 2>&1 <==> |&
    new_installed_version=$(nginx -V 2>&1 | sed -r -n '1s@.*/([[:digit:].]*)@\1@p')
    ...
    if [[ -f "${default_conf_path}" && ! -f "${default_conf_path}${bak_suffix}" ]]; then
        cp -f "${default_conf_path}" "${default_conf_path}${bak_suffix}"
        $download_tool "${conf_default_conf}" > "${default_conf_path}"
    fi
    ...
    funcSystemServiceManager 'nginx' 'enable'
