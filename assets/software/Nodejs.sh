...
#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'NJTemp_XXXXXX'}
...
funcVersionLocalCheck(){
    if [[ -f "${installation_dir}/bin/node" ]]; then
        is_existed=1
        current_version_local=$("${installation_dir}/bin/node" --version | sed -r -n 's@v([[:digit:].]*)@\1@p')
    fi
}
...
funcDownloadAndDecompressOperation(){
    ...
    if [[ -f "${pack_save_path}" && -f "${sha256_save_path}" ]]; then
        ...
        cd "${temp_save_path}" || exit
        grep "${pack_save_path##*/}" "${sha256_save_path##*/}" | sha256sum -c -- &> /dev/null
        if [[ $? -eq 0 ]]; then
    ...
    new_installed_version=$("${installation_dir}/bin/node" --version | sed -r -n 's@v([[:digit:].]*)@\1@p')    # Just Installed Version In System
