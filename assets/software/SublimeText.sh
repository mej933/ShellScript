...
#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'SLTTemp_XXXXXX'}
...
readonly installation_dir="/opt/${application_name}"      # Decompression & Installation Path Of Package
...
funcVersionLocalCheck(){
    if [[ -s "${installation_dir}/sublime_text" ]]; then
        is_existed=1
        # Sublime Text Build 3143
        current_version_local=$("${installation_dir}/sublime_text" -v | sed -r -n 's@[^[:digit:]]+([[:digit:]]*).*@\1@g;p')
    fi
}
