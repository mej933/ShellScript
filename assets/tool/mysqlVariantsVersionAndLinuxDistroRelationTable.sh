...
#Writer: MaxdSre
#Date: Nov 15, 2017 19:35 Wed +0800
#Update Time:
...
#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'MVVALDRTemp_XXXXXX'}
...
readonly relation_script_path="${custom_shellscript_url}/master/assets/gnulinux/gnuLinuxLifeCycleInfo.sh"
...
[available option]
    ...
    -c    --add cluster revelant info (Percona XtraDB Cluster), default is not list
...
funcPerconaDistroListForPerVersion(){
    ...
    if [[ -n "${item}" ]]; then
        ...
        echo "${item}" | while IFS="|" read -r item1 item2 item3; do
            ...
            distro_list=$($download_tool "${db_version_url}" | sed -r -n '/Select Software Platform/{s@redhat/@rhel@g;s@<\/option>@\n@g;p}' | sed -r -n 's@.*/([^"]*)"[[:space:]]*>.*@\1@g;/binary|source|select|^$/d;p' | sed -r ':a;N;$!ba;s@\n@ @g;')
...
funcPerconaOperation(){
    ...
    $download_tool "${download_page}" | sed -r -n '/<div class="nli-title">/,/^[[:space:]]*<\/div>[[:space:]]*$/{/for MongoDB/,$d;/<a/{s@.* href="([^"]*)".*>Download ([[:digit:].]+).*@\2|'"${official_site}"'\1@g;/Percona-Server/s@.*@Percona Server|&@g;/Percona-XtraDB-Cluster/s@.*@Percona XtraDB Cluster|&@g;;p}}' | parallel -k -j 0 funcPerconaDistroListForPerVersion 2> /dev/null
    ...
    sed -r 's@.*\|@@g' "${version_list_info}" | sed -r ':a;N;$!ba;s@\n@ @g;s@ @\n@g' | awk '{a[$0]++}END{PROCINFO["sorted_in"]="@ind_str_asc";for (i in a) print i}' | parallel -k -j 0 funcPerconaVersionListPerDistro | sort -b -f -t"|" -k 1,1 -k 2r,2 2> /dev/null
...
funcMySQLOperation(){
    ...
    # funcMySQLForRPM 'yum' >> "${mysql_list_info}"
    # funcMySQLForRPM 'suse' >> "${mysql_list_info}"
    { funcMySQLForRPM 'yum'; funcMySQLForRPM 'suse'; } >> "${mysql_list_info}"
    funcMySQLForDEB >> "${mysql_list_info}"
