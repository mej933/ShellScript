...
#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'PIETemp_XXXXXX'}
...
funcPremiumProxySocks(){
    ...
    page_no=$($download_tool "${page_url}" | sed -r -n '/pagination/{s@next@@g;s@[[:space:]]*<[^>]*>[[:space:]]*@ @g;s@[[:space:]]+@ @g;s@.*[[:space:]]+([[:digit:]]+)[[:space:]]*$@\1@g;p}')

    if [[ -z "${page_no}" ]]; then
        echo -e "Fail to extract info from site ${c_blue}${page_url}${c_normal}"
    else
        local flag
        flag=${flag:-"${page_no}"}
        [[ "${page_no}" -ge 10 ]] && flag=9
